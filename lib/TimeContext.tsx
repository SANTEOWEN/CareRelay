...
export const TimeProvider: React.FC<TimeProviderProps> = ({ children }) => {
    ...
    const clockOut = () => {
        ...
        if (currentSession.timeIn) {
            const completedSession: Session = {
                ...
                status: 'out',
                timeOut: now,
                totalHours: (now.getTime() - currentSession.timeIn.getTime() - currentSession.totalBreakTime) / (1000 * 60 * 60),
    ...
    const getTodayWorkSummary = (): {hours: number; minutes: number} => {
        ...
        const totalMs: number = todaySessions.reduce((sum: number, s: Session) => {
        const workMs = (s.timeIn && s.timeOut ? s.timeOut.getTime() - s.timeIn.getTime(): 0) - s.totalBreakTime;
        return sum + workMs;
        }, 0);
